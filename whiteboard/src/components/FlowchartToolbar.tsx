import { Square, Type, Diamond, Circle, Hexagon, MousePointer2, ArrowRight, PenLine, StickyNote, Wand2, Eraser } from 'lucide-react';
    ...
    return (
        <div className="fixed left-6 top-1/2 -translate-y-1/2 z-10 flex flex-col items-center gap-2 bg-white/90 backdrop-blur-xl px-2 py-3 rounded-2xl shadow-xl border border-slate-200/80 drop-shadow-sm transition-all hover:shadow-md">
            ...
            <button
                ...
                className={`p-2.5 rounded-xl transition-colors ${mode === 'eraser' ? 'bg-pink-100 text-pink-600 shadow-sm' : 'text-slate-500 hover:text-pink-500 hover:bg-slate-100'}`}
                title="Eraser"
            >
                <Eraser size={20} />
            </button>
